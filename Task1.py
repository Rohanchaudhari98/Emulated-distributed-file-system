...
# db = pymysql.connect(host="127.0.0.1",user = "root",password= "Qwerty123",database = "edfs",charset='utf8')
# cursor = db.cursor()
...
db = pymysql.connect(host="127.0.0.1",user = "root",password= "",database = "edfs" )
cursor = db.cursor()
    ...
    if request.method == "PUT":
        ...
        if temp[1] == "csv" and temp[1] == "txt":
            ...
            #current_id=1
                ...
                cursor.execute(sql)
                db.commit()
                sql="select current_id from FS_Structure where name='%s'" %(name)
